...
export async function writeAudit(entry: { userId?: number | null; userName: string; userEmail?: string | null; role?: string | null; action: string; module?: string | null; details?: string | null; ipAddress?: string | null }) {
...
export async function auditAction(request: Request, actor: AuditActor, action: string, moduleName: string | null, details?: string | null) {
  await writeAudit({
    userId: actor.id,
    userName: actor.name,
