...
import { Crosshair, LoaderCircle, MapPin, Pencil, RotateCcw, Save, Search, TriangleAlert, X } from "lucide-react";
...
export function OfficerPortfolioMap({ officerId, officerName, locations, isAdmin }: { officerId: number; officerName: string; locations: OfficerMapLocation[]; isAdmin: boolean }) {
  ...
  function useCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      setError(locationError.code === 1 ? "Location permission was denied. Allow location access in the browser and try again." : locationError.code === 3 ? "Location capture timed out. Move to an open area and try again." : "Your current location could not be determined.");
      setLocating(false);
