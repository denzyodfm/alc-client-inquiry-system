...
type AppFunction = { key: string; label: string; description: string; adminOnly?: boolean };
...
export function AccessControlMatrix({ privileges, functions }: { privileges: Privilege[]; functions: AppFunction[] }) {
  ...
  function reportRows() {
    return functions.map((item) => [item.label, ...privileges.map((privilege) => item.adminOnly ? "ADMIN ONLY" : checked.has(`${privilege.id}:${item.key}`) ? "ON" : "OFF")]);
  }
  ...
  async function save() {
    ...
    try {
      const matrix = privileges.map((privilege) => ({
        ...
        functionKeys: functions.filter((item) => !item.adminOnly && checked.has(`${privilege.id}:${item.key}`)).map((item) => item.key)
      }));
