...
export function DashboardClientLogs({ rows, canDelete = false, initialSearch = "" }: { rows: Row[]; canDelete?: boolean; initialSearch?: string }) {
  ...
  return <div className="space-y-3">{error && !editingRow ? <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{error}</div> : null}<div className="flex items-center justify-between gap-2 print:hidden"><span className="text-xs font-semibold text-slate-500">Showing {visibleRows.length} of {rows.length} log(s)</span><div className="flex gap-2"><button className="btn-secondary px-3 py-1.5 text-xs" onClick={() => window.print()}><Printer className="h-4 w-4" />Print</button><button className="btn-secondary px-3 py-1.5 text-xs" onClick={excel}><FileSpreadsheet className="h-4 w-4" />Excel</button></div></div><details className="panel px-3 py-2 text-xs" open><summary className="cursor-pointer font-bold text-slate-900">Report summary<span className="ml-2 font-semibold text-slate-500">{summary.clients.toLocaleString("en-US")} client(s), {visibleRows.length.toLocaleString("en-US")} log(s), {peso(summary.ptpTotal)} promised</span></summary>
