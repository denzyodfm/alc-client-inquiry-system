...
export function CurrentDetailReport({
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const haystack = [row.clientName, row.clientId, row.branchName, row.loanNumber, row.loanProduct]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  function downloadExcel() {
    const reportRows = filteredRows
      ...
      <tr>
        ...
        <td>${escapeHtml(dateOnly(row.releasedAt))}</td>
        <td>${escapeHtml(dateOnly(row.maturityAt))}</td>
    ...
    const workbook = `<!doctype html><html><head><meta charset="utf-8"><style>
      ...
      <table>
        <thead><tr>
          ...
          <th>Released</th><th>Maturity</th><th>Due Today</th><th>Paid</th><th>Balance</th>
        </tr></thead>
        <tbody>${reportRows || '<tr><td colspan="11">No current loans found.</td></tr>'}</tbody>
      </table>
    </body></html>`;
    ...
    link.href = url;
    link.download = `${title.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase() || "current-loans"}.xls`;
        ...
        <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 px-5 py-3 no-print">
          <label className="relative block min-w-[240px] flex-1">
            ...
            <input
              ...
              placeholder="Search client, client ID, branch, loan no., or product"
            />
          </label>
          <button
            type="button"
              ...
              {!filteredRows.length ? (
                <tr>
                  <td className="px-4 py-6 text-slate-500" colSpan={10}>
                    {rows.length ? "No loans match your search." : "No current loans found."}
                  </td>
                </tr>
              ) : null}
            </tbody>
