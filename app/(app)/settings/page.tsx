...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <div className="space-y-4">
    ...
    {activeTab === "users" ? <UserManager initialUsers={users.map(({ area, baseBranch, ...user }) => ({ ...user, area: area ? { id: area.id, name: area.name, areaTeamLeaderName: area.areaTeamLeader?.name ?? null } : null, baseBranch: baseBranch ? { id: baseBranch.id, branchName: baseBranch.branchName, branchCode: baseBranch.branchCode, branchTeamLeaderName: baseBranch.branchTeamLeader?.name ?? null } : null }))} branches={userBranches.map(({ id, branchName, branchCode, branchTeamLeader }) => ({ id, branchName, branchCode, branchTeamLeaderName: branchTeamLeader?.name ?? null }))} currentUserRole={currentUser.role} canGrantAllBranches={isAdmin || accessibleBranchIds === null} privileges={privilegeOptions} areas={areaOptions} teamLeaders={teamLeaderOptions} branchTeamLeaders={branchTeamLeaderOptions} /> : null}
