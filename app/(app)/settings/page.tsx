...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const [allBranches, privileges, areas, users, userBranches, syncLogs, auditLogs, footerBranding, verificationBaseline] = await Promise.all([
    ...
    canUsers ? prisma.user.findMany({
      ...
      orderBy: { name: "asc" },
      select: { id: true, name: true, userCode: true, email: true, role: true, position: true, baseBranchId: true, allBranches: true, isActive: true, privilegeTemplateId: true, privilegeTemplate: { select: { id: true, name: true } }, areaId: true, area: { select: { id: true, name: true, areaTeamLeader: { select: { name: true } } } }, areaTeamLeaderId: true, areaTeamLeader: { select: { id: true, name: true } }, branchTeamLeaderId: true, branchTeamLeader: { select: { id: true, name: true } }, baseBranch: { select: { id: true, branchName: true, branchCode: true, branchTeamLeader: { select: { name: true } } } }, branchAccess: { select: { branchId: true } } }
    }) : [],
