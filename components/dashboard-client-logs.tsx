...
import { CalendarDays, FileSpreadsheet, Pencil, Printer, Save, Trash2, X } from "lucide-react";
import { useRouter } from "next/navigation";
import { useEffect, useMemo, useState } from "react";

type Row = { id: number; client: string; clientNumber: string | null; branch: string; accountOfficer: string; type: string; subject: string | null; notes: string; newDate: string | null; newAmount: string | null; visitAt: string };
...
export function DashboardClientLogs({ rows, canDelete = false, initialSearch = "" }: { rows: Row[]; canDelete?: boolean; initialSearch?: string }) {
  ...
  return <div className="space-y-3">{error && !editingRow ? <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{error}</div> : null}<div className="flex items-center justify-between gap-2 print:hidden"><span className="text-xs font-semibold text-slate-500">Showing {visibleRows.length} of {rows.length} log(s)</span><div className="flex gap-2"><button className="btn-secondary px-3 py-1.5 text-xs" onClick={() => window.print()}><Printer className="h-4 w-4" />Print</button><button className="btn-secondary px-3 py-1.5 text-xs" onClick={excel}><FileSpreadsheet className="h-4 w-4" />Excel</button></div></div><div className="panel max-h-[calc(100vh-24rem)] min-h-[360px] overflow-auto"><table className="w-full min-w-[1000px] text-xs"><thead className="sticky top-0 bg-slate-50"><tr><th className="px-3 py-2 text-left">Date</th><th className="px-3 py-2 text-left">Client</th><th className="px-3 py-2 text-left">Branch</th><th className="px-3 py-2 text-left">Account Officer</th><th className="px-3 py-2 text-left">Type</th><th className="px-3 py-2 text-left">Activity</th>{canDelete ? <th className="px-3 py-2 text-right print:hidden">Action</th> : null}</tr></thead><tbody>{visibleRows.map((row) => <tr key={row.id} className="border-t border-slate-100"><td className="whitespace-nowrap px-3 py-2">{new Date(row.visitAt).toLocaleString("en-US")}</td><td className="px-3 py-2"><b>{row.client}</b><br /><span className="text-slate-500">{row.clientNumber ?? "-"}</span></td><td className="px-3 py-2">{row.branch}</td><td className="px-3 py-2">{row.accountOfficer}</td><td className="px-3 py-2">{row.type.replace(/_/g, " ")}</td><td className="px-3 py-2"><b>{row.subject}</b>{row.subject ? <br /> : null}{row.notes}</td>{canDelete ? <td className="px-3 py-2 text-right print:hidden"><div className="flex justify-end gap-1"><button type="button" className="btn-secondary h-8 px-2 text-xs" onClick={() => beginEdit(row)}><Pencil className="h-3.5 w-3.5" />Edit</button><button type="button" className="inline-flex h-8 items-center gap-1 rounded-md border border-red-200 px-2 font-semibold text-red-600 hover:bg-red-50 disabled:opacity-50" onClick={() => remove(row)} disabled={deletingId === row.id}><Trash2 className="h-3.5 w-3.5" />{deletingId === row.id ? "Deleting..." : "Delete"}</button></div></td> : null}</tr>)}{!visibleRows.length ? <tr><td colSpan={canDelete ? 7 : 6} className="p-8 text-center text-slate-500">No client logs match the filters.</td></tr> : null}</tbody></table></div>
