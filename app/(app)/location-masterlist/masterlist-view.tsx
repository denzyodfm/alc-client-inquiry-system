...
// Every pivot reads the same loans, so they share one loader and one set of row components.
// Which of them a page draws is the only difference between the routes under this folder.
export type MasterlistView = "location" | "branch" | "officer" | "portfolio" | "log";

const viewIntro: Record<MasterlistView, { title: string; blurb: string }> = {
  location: { title: "Location Pivot", blurb: "Live outstanding-loan portfolio grouped by linked province, city/municipality, and barangay." },
  branch: { title: "Branch Pivot", blurb: "The same portfolio read branch first, then province, city/municipality and barangay." },
  officer: { title: "Officer Location Pivot", blurb: "Team leaders and their Loan / Remedial Officers, and the places their assigned loans sit in." },
  portfolio: { title: "Officer Portfolio by Location", blurb: "What each Loan / Remedial Officer is carrying, by place." },
  log: { title: "Location Linking Log", blurb: "History of the runs that link loans to a barangay in the masterlist." }
};

export async function LocationMasterlistView({ view }: { view: MasterlistView }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-brand-green">Location Masterlist</p>
          <h2 className="mt-2 text-3xl font-bold text-slate-950">{viewIntro[view].title}</h2>
          <p className="mt-2 text-sm font-semibold text-slate-600">{viewIntro[view].blurb}</p>
        </div>
