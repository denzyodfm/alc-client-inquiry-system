...
export function AgingReportFilter({
  branches,
  selectedBranchId,
  searchText
}: {
  ...
  return (
    <form onSubmit={submit} className="panel grid gap-3 p-4 md:grid-cols-[1fr_2fr_auto_auto]">
      ...
      <label className="block">
        <span className="mb-2 block text-sm font-semibold text-slate-700">Search aging report</span>
        <input
          name="q"
          className="field"
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Client, address, client no., or loan no."
        />
      </label>
      ...
      <Link className="btn-secondary self-end" href="/aging">
        Clear
      </Link>
    </form>
  );
