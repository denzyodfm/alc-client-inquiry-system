...
# The generated Prisma client is not in git and is not refreshed by `prisma migrate deploy`, so
# after a schema change the build here type-checks against a client that has never heard of the
# new table. That is not a hang and no amount of retrying fixes it: on 7 Sep it failed every
# attempt with "build worker exited with code 1" until the client was regenerated by hand.
echo "==> Regenerating the Prisma client"
npx prisma generate >/dev/null 2>&1 && echo "    ok" || echo "    prisma generate failed - the build will probably fail too" >&2
