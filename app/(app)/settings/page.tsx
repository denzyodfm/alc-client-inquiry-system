...
type Tab = "general" | "branches" | "areas" | "privileges" | "matrix" | "users" | "sync-logs" | "system-logs" | "change-password";
...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const [allBranches, privileges, areas, users, userBranches, syncLogs, auditLogs, footerBranding] = await Promise.all([
    ...
    canUsers ? prisma.user.findMany({
      ...
      orderBy: { name: "asc" },
      select: { id: true, name: true, email: true, role: true, position: true, baseBranchId: true, allBranches: true, isActive: true, privilegeTemplateId: true, privilegeTemplate: { select: { id: true, name: true } }, areaId: true, area: { select: { id: true, name: true, areaTeamLeader: { select: { name: true } } } }, areaTeamLeaderId: true, areaTeamLeader: { select: { id: true, name: true } }, baseBranch: { select: { id: true, branchName: true, branchCode: true } }, branchAccess: { select: { branchId: true } } }
    }) : [],
  ...
  const branchTeamLeaderOptions = canBranches ? await listBranchTeamLeaders() : [];
  ...
  return <div className="space-y-4">
    ...
    {activeTab === "users" ? <UserManager initialUsers={users.map(({ area, ...user }) => ({ ...user, area: area ? { id: area.id, name: area.name, areaTeamLeaderName: area.areaTeamLeader?.name ?? null } : null }))} branches={userBranches} currentUserRole={currentUser.role} canGrantAllBranches={isAdmin || accessibleBranchIds === null} privileges={privilegeOptions} areas={areaOptions} teamLeaders={teamLeaderOptions} /> : null}
